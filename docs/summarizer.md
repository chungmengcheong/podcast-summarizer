...
For monitored episodes, `show` comes from the matching configured show. For a
one-off record with `show_id: "user-injected"`, it comes from that record's
`show_display_name`; this starts as `User-injected` and is replaced with the
YouTube channel name when available during metadata enrichment.
